...
images_path = r'/home/rm/realman_ws/src/rm_scr_robot/rm_scr_demo/scripts/images' #手眼标定采集的标定版图片所在路径
file_path = r'/home/rm/realman_ws/src/rm_scr_robot/rm_scr_demo/scripts/images/poses.txt'   #采集标定板图片时对应的机械臂末端的位姿 从 第一行到最后一行 需要和采集的标定板的图片顺序进行对应

def func():
    path = os.path.dirname(__file__)

    # 角点的个数以及棋盘格间距
    XX = 4 # 11 #标定板的中长度对应的角点的个数
    YY = 4 # 8  #标定板的中宽度对应的角点的个数
    ...
    tool_pose = np.loadtxt(f'RobotToolPose.csv',delimiter=',')
    R_tool = []
    t_tool = []
