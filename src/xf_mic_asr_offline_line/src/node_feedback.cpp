/**
 * 功能：反馈控制节点，处理雷达跟随、色块跟随、自主建图、导航的开启/关闭状态反馈，
 *       接收关闭指令并播放对应语音反馈，同时打印状态日志
 * 		功能与command_recognition.cpp中的指令回调函数功能相同,可以合并
 */
#include <ros/ros.h>
#include <signal.h>
#include <stdlib.h>
...
/**************************************************************************
函数功能：语音关闭指令回调函数
入口参数：msg - 订阅到的语音命令（String类型，如"关闭雷达跟随"）
          消息来源：cmd_rec节点发布的"voice_words"话题
返回  值：无
核心逻辑：匹配关闭类命令词，播放对应关闭反馈语音，打印关闭成功日志
**************************************************************************/
...
返回  值：0（程序正常退出）
核心逻辑：初始化节点、创建订阅者、获取音频路径参数、启动事件循环
**************************************************************************/
int main(int argc, char** argv)
{
	...
	nha.param("/node_feedback/audio_path", audio_path, std::string("~/realman_ws/src/xf_mic_asr_offline_line/feedback_voice"));
