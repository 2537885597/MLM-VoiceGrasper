...
// #include <xf_mic_asr_offline_line/Set_Awake_Word_srv.h>
...
int confidence_threshold ;		// 语音识别置信度
int seconds_per_order ;			// 单词命令识别超时时间
...
{
	awake_flag = msg.data;		// 更新全局唤醒状态
	printf("awake_flag=%d\n",awake_flag);
	recognize_fail_count = 0;	// 更新休眠相关状态
...
int main(int argc, char *argv[])
{
	...
	// SetAwakeWord_srv.request.awake_word="你好小微";
	...
	while(ros::ok())
	{
		...
		ros::spinOnce();    	//单次处理所有待处理的 ROS 回调 非阻塞式
